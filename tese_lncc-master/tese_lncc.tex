...
\begin{resumo}[Abstract]
 ...
 % Proposal
 In this work we propose a consistent methodology for spatio--temporal predictive query [evaluation], which aims to reduce the computational workload and time that would be consumed if we were to train a model on each element of a spatio–temporal domain, while maintaining comparable performance of the models that attend the query. Our methodology has the following steps (1) the domain characterization process, based on clustering partition techniques to find representative elements; (2) the generation of predictive models for the representatives; (3) a knowledge extraction process for the underlying relationship between the representative models and domain characterization; and (4) a mechanism to compute a spatio--temporal predictive query.  

 %Results
 In order to evaluate the applicability of this methodology to generate predictive models in a spatio--temporal domain for query processing, we use a case study for weather forecasting. Results from computational experiments show that we can achieve comparable predictive performance using models trained with the representatives selected by the knowledge extraction mechanism, with a fraction of the computational cost. 
