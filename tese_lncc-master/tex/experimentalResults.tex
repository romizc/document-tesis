...
The calculation of the in-sample error corresponds to the `Evaluate In-Sample Error' item presented in Step Two of the methodology (Figure \ref{Fig:OverviewMethodology} in Chapter \ref{Chapter:Methodology}). The additional experiments are not required to produce a model composition that is capable of processing predictive queries; but will allow us to better assess the accuracy of the models and justify aspects of the methodology. This analysis can be described as creating an intermediate process between Step Two and Step Three (see Figure \ref{Fig:MethodologyExperiments}). 
...
To qualify the predictive power of the representative predictor forecast error for each individual element of its group, we compute the sMAPE error of the prediction from the representative model for each element.%, given that we know the corresponding future values. 
We perform an exploratory analysis using scatter plots diagrams that aggregate the sMAPE results, shown in Figures \ref{Fig:DTWsMAPE_k8_c0}-\ref{Fig:DTWsMAPE_k66_c24}. % for each representative in a given partitioning scheme.
...
% Objectivo: Analizar el error de forecast del medoide para predecir su propio grupo (error de forecast acumulado MSE).
...
\begin{table}[h]
	...
	\begin{tabular}{|c|r|r|r|r|r|r|r|r|r|}
		\hline
		cid & size & $(p, d, q)$ & T. time (s) & F. time(s) & Each & Min. & Min. Local & Medoid & Max. \\
		\hline
        0 &  574 & (0, 1, 2) &  2041.469   & 1.069    & 0.170 & 0.161 & 0.289 & 0.185 & 0.438  \\
        1 &  817 & (2, 1, 2) &  3447.608   & 1.299    & 0.689 & 0.460 & 0.531 & 0.926 & 1.566  \\
        2 &  542 & (1, 1, 2) &  2011.441   & 0.880    & 0.581 & 0.514 & 0.577 & 0.678 & 1.420  \\
        3 &  755 & (1, 1, 1) &  2685.912   & 1.238    & 0.413 & 0.289 & 0.364 & 0.492 & 0.878  \\
        4 & 3479 & (1, 1, 2) & 14542.318   & 5.727    & 0.785 & 0.475 & 0.548 & 0.838 & 1.983  \\
        5 &  803 & (1, 1, 1) &  3231.718   & 1.375    & 0.407 & 0.294 & 0.378 & 0.437 & 1.194  \\
        6 &  625 & (3, 1, 1) &  1930.740   & 0.957    & 0.157 & 0.168 & 0.175 & 0.203 & 0.478  \\
        7 &  505 & (3, 1, 1) &  1811.335   & 0.853    & 0.388 & 0.375 & 0.422 & 0.551 & 1.015  \\ \hline	
	\end{tabular}
...
\begin{table}[h]
	...
	\begin{tabular}{|c|r|r|r|r|r|r|r|r|r|}
		\hline
        cid & size & $(p, d, q)$ & T. time (s) & F. time (s) & Each & Min. & Min. Local & Centroid & Max. \\
        \hline
        0 &  990 & (2, 1, 2) &  3601.959 & 1.642 & 0.333 & 0.305 & 0.379 & 0.331 & 0.826 \\
        1 &  990 & (0, 1, 2) &  3221.489 & 1.665 & 0.192 & 0.180 & 0.198 & 0.268 & 0.517 \\
        2 &  990 & (1, 1, 2) &  4063.693 & 1.621 & 0.384 & 0.460 & 0.888 & 0.587 & 1.608 \\
        3 &  990 & (2, 1, 1) &  3424.456 & 1.610 & 0.341 & 0.387 & 0.634 & 0.394 & 1.028 \\
        4 &  990 & (2, 1, 2) &  4305.636 & 1.651 & 0.782 & 0.571 & 1.182 & 0.769 & 3.058 \\
        5 &  990 & (1, 1, 2) &  3818.287 & 1.713 & 0.470 & 0.489 & 0.580 & 0.609 & 2.062 \\
        6 & 1080 & (0, 1, 0) &  4674.208 & 1.744 & 1.190 & 0.564 & 0.862 & 2.454 & 2.935 \\
        7 & 1080 & (2, 1, 1) &  4847.003 & 1.795 & 0.546 & 0.463 & 0.563 & 0.583 & 1.188 \\ \hline
	\end{tabular}
...
\begin{figure}[!htbp]
  ...
  \begin{minipage}[b]{0.45\textwidth}
    ...
    \caption{Forecast Error with Model Composition by Representatives for kMedoids ($k=8$).}
    \label{Fig:kMedoid_10x10_k8}
  \end{minipage}
  \hfill
  \begin{minipage}[b]{0.45\textwidth}
    \includegraphics[width=\textwidth]{../Figures/query_10x10_regular_k8-1000dpi}
    \caption{Forecast Error with Model Composition by Representatives for Regular ($k=8$).}
...
%\begin{figure}[!tbp]
  ...
  \begin{minipage}[b]{0.45\textwidth}
    ...
    \caption{Forecast Error with Model Composition by Representatives for kMedoids ($k=66$).}
    \label{Fig:kMedoid_10x10_k66}
  \end{minipage}
  \hfill
  \begin{minipage}[b]{0.45\textwidth}
    \includegraphics[width=\textwidth]{../Figures/query_10x10_regular_k66-1000dpi}
    \caption{Forecast Error with Model Composition by Representatives for Regular ($k=66$).}
...
%\begin{figure}[!tbp]
  ...
  \begin{minipage}[b]{0.45\textwidth}
    ...
    \caption{Forecast Error with Model Composition by Representatives for kMedoids ($k=132$).}
    \label{Fig:kMedoid_10x10_k132}
  \end{minipage}
  \hfill
  \begin{minipage}[b]{0.45\textwidth}
    \includegraphics[width=\textwidth]{../Figures/query_10x10_regular_k132-1000dpi}
    \caption{Forecast Error with Model Composition by Representatives for Regular ($k=132$).}
    \label{Fig:Regular_10x10_k132}
  \end{minipage}
\end{figure}

Experimentally, we observe that for different values of $k$, the composition of models also presents variations in the prediction values throughout the different slices, this is due to the properties of spatio-temporal data. We further note that there is a particular spatial region near the bottom left, where the forecast error tends to be larger. Also, lower values of $k$ (8, 66) may behave better than $k=132$ for some of the slices. 
...
According to the proposed methodology, we want to implement an algorithm that allows classifying the domain elements in a set of classes. These classes are determined by the representative and the elements are those that present the minimum similarity (DTW distance) between them.

Considering the partition schemes of sizes $k=\{8, 66, 132\}$, we obtain $183$ classes in total, after accounting for repetition of medoids between the schemes. In order to work with a balanced dataset, we extract approximately $30$ examples per class \cite{Du2018}. In total we consider 5000 examples, we divide the dataset in the percentages $60/20/20$ for training, validation and test, respectively.
...
% Scaling data: In the experiment, the neural networks considered are trained with the dataset scaled, the network performance is evaluated on the test set.
...
The results of training the classifier models are shown in Table \ref{Table:DLModels}. The accuracy of the first two models was very low, which means that the model was not able to predict the label for a new time series. In fact, we verified that these two models only learned one class, producing the same output for all tried inputs.
...
In Section \ref{Sec:ModelCompositionAggregated} from \ref{Sec:AnalyzeForecastErrors}, we evaluate the predictive quality 
...
\begin{figure}[h!]
	\subfloat[$k$--Medoids.]{
		\begin{minipage}[c][1\width]{
				0.5\textwidth}
			\centering
			\includegraphics[width=1\textwidth]{../Figures/query_10x10_solver-multiclass-1000dpi}
	\end{minipage}}
	\hfill 	
	\subfloat[Regular.]{
		\begin{minipage}[c][1\width]{
				0.5\textwidth}
			\centering
			\includegraphics[width=1\textwidth]{../Figures/query_10x10_solver-each-1000dpi}
	\end{minipage}}
