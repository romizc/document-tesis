...
\subsection{Analyzing the Predictive Quality of Models on Representatives}
...
% Predictor ARIMA
In Step Two of our proposed methodology (Section \ref{Sec:ModelRepresentatives}), we generate an ARIMA model for each representative obtained from the domain partitioning. Once an appropriate time series model has been fit, it may be used to generate forecasts of future observations. A prediction interval, a similar concept to a confidence interval, is an estimate of the range of likely values that are not yet known but are going to be observed at some point in the future. A prediction interval needs to take into account uncertainty in the model, uncertain estimates of the parameters in a model (i.e. the confidence intervals for those parameters), and also the individual randomness associated with the particular point or points being predicted \cite{Chatfield2001}.
...
\begin{equation}
%\mathcal{S}^{*} = \lbrace s_0, s_1, \ldots s_{364} \rbrace
\lbrace s_t \rbrace = \lbrace s_0, s_1, \ldots s_{364} \rbrace
\end{equation}
...
To evaluate an ARIMA model $g_{\mathcal{S}^{*}}$ and its predictive power, the series is then split into three parts. The first part $s^{tr}_t = \lbrace s_0, s_1, \ldots s_{349} \rbrace$ is used for training the model, %($s^t_r[0:349]$ as an example in Figure \ref{Fig:Time-SeriesSplit})
here we determine the tuple $(p_{\mathcal{S}^{*}}, d_{\mathcal{S}^{*}}, q_{\mathcal{S}^{*}})$ that corresponds to the model $g_{\mathcal{S}^{*}}$. Then, we take the next $t_p$ data points ($t_p = 8$ in our example) to create the test series $s^{ts}_t = \lbrace s_{349}, \ldots s_{356} \rbrace$.
%$s^v_t[349:357]$.
The purpose of this split is to obtain the in-sample error ${E^*}$ of the model: this is achieved by using the model $g_{\mathcal{S}^{*}}$ to make a prediction of $t_p$ steps, and then calculating an associated error using one of the error expressions presented in Section \ref{Sec:ModelRepresentatives}. This corresponds to the Generalization Error presented in Step Two of the proposed methodology (Figure \ref{Fig:OverviewMethodology} in Chapter \ref{Chapter:Methodology}).
...
For ARIMA models we use the sMAPE measure to describe how well a model fits a given sample of data. This goodness-of-fit approach often uses the residuals and does not really reflect the capability of the forecasting technique to successfully predict future observations \cite{Montgomery2015}. The user of the forecasts is very concerned about the accuracy of future forecasts, not model goodness of fit, so it is important to evaluate this aspect. Sometimes forecast accuracy is called `out-of-sample' forecast error, to distinguish it from the residuals that arise from a model-fitting process. In the following Section, we analyze the forecast error of the models on the representatives.

\subsubsection{Forecast Error of Models on Representatives}
\label{Sec:AnalyzeForecastErrors}

% Objetivo: Analizar el error de forecast del medoide aplicado sobre cada uno de sus elementos en su grupo. 
% Between Step 2 and 3 we consider an intermediate analysis, focused on the forecast error of the model on the predictor, given that the out-of-sample interval ....
The previous section described the process of calculating a multi-step prediction for the future values of a time series, using a predictive model trained at a representative. Given that the out-of-sample interval of length $t_f$ is known from the dataset, we can then present results on the corresponding forecast error. 
% We consider the following analysis, forecast error variation of the model on the representative applied to each element in its group (for both domain partitioning techniques).
This is related to the Forecast Error presented in Step Three of the proposed methodology (Figure \ref{Fig:OverviewMethodology} in Chapter \ref{Chapter:Methodology}).

% Now, it is important to remember that there are many different possible model compositions, reviewed in Section \ref{Sec:KnowledgExtraction}. In this Section, we focus in the `Model Composition at the Medoids' to show how forecast errors are calculated for a group of elements based on a model calculated at the corresponding representative. The next Section will later include the other model compositions when forecast errors are aggregated.

We begin by analyzing the forecast errors when using $k$--Medoids as a partitioning scheme (which yields the medoids as representatives) and the auto ARIMA procedure as the predictive model. In the case of a $k=8$, we have eight groups with the number of elements varying from 500 to 3479, representing the $6.2\%$ and $43\%$ of the domain, respectively. This means that there are eight possible predictive models that we can use to make predictions for any element in the domain.

By considering a larger value for the number $k=66$, we are creating more groups and therefore more representatives that generalize better the elements in its group. The partitioning domain result are groups varying from 14 to 278 elements, representing the $0.2\%$ and $3.4\%$ of the domain respectively. 
...
However, an important observation here is that lower values of $k$ (8, 66) are still able to produce some representatives that offer better predictions than, for example, the `worst' (highest forecast error) representatives of the partitioning scheme with $k=132$. This result justifies the inclusion of the classifier in our methodology, because there is potential for improved accuracy when considering representatives from multiple partitioning schemes.

\newpage
A similar analysis is performed with the regular partitioning domain, we generate scatter plots figures for the distance DTW from the medoid and forecast erros (in sMAPE). For different domain partitioning schemes ($k=\{8, 66, 132\}$), we show in the left column the `best' representatives, those for which the forecast error is inside the prediction intervals of the representatives (See Section \ref{sec:InSampleForecastErrors}), and in the right column are the representative for which the forecast error is outside the prediction interval. 
...
In this case, we can observe that the density of the groups is more expanded and the variation of the forecast error is also grater. Which also help us to evaluate the advantage of grouping the domain under the similarity based on the shape of its elements, rather than by a mere division based on the geometry of the domain.

%It is not feasible to find a direct correlation between DTW distance and forecast error (and also in-sample error), this is due to the nature of the ARIMA models (elaborate more). To establish a more generic function for the forecast error, or the predictive power, we have to consider 
% TODO Add reference for the statistical equivalent of the learning function.

\subsection{Predictive Quality Analysis of a Model Composition}
\label{Sec:MedoidsModelComposition}

Now, it is important to remember that there are many different possible model compositions, reviewed in Section \ref{Sec:KnowledgExtraction}. In this Section, we focus in the `Model Composition at the Medoids' to show how forecast errors are calculated for a group of elements based on a model calculated at the corresponding representative. The next Section will later include the other model compositions when forecast errors are aggregated.

% Objectivo: Analizar el error de forecast del medoide para predecir su propio grupo.
In this Section we describe the experiments performed to evaluate the forecast error of a model composition, as discussed in Section \ref{Sec:KnowledgExtraction} of the methodology. A model composition is formed by one or more predictors which will be used to perform a prediction on a domain region. Recalling from Section \ref{Sec:ModelRepresentatives}, the model compositions of interest are:
...
For the experiments, we also compute the elapsed time for training the models for all the elements in the domain, by doing that we are able to quantify the a metric for the computational effort of each approach.
%cuanto tiempo a menos seria usado en la metodologia propuesta. 
...
% Objetivo 2: Analizar el error de forecast del predictor cuando la region de interes tiene mas de un medoide
...
According to Table \ref{Table:Query10x10_kMedoids_Regular_StatSummary}, the model composition formed by representative models of the $k$--Medoids representatives predict better than the regular approach. 
...
\subsubsection{Classifier Architectures and Results}
...
In this Section we show the experiments and results for the on--line procedure implemented to execute a spatio-temporal predictive query (Section \ref{Sec:SpatioTemporalQueryProcessing}). Recalling from Section \ref{Sec:ProblemFormalization}, a spatio-temporal predictive query is denoted by the tuple:
...
% Adicionalmente, para evaluar la aplicabilidad de la metodologia computamos las consultas predictivas espacio-temporales considerando modelos baseline tal como $kNN$.
...
We describe the results of the experiments considering queries with different sizes and over different regions of the domain. We have considered $t_{p} = 8$ and $t_{f} = 8$. For the model composition we consider the following type of models:
...
In Section \ref{Sec:Aggregated} from \ref{Sec:AnalyzeForecastErrors}, we evaluate the predictive quality 
% consideramos evaluar la calidad predictiva de la composicion de modelos sobre los medoids sobre regiones de tamanho $10\times 10$. En esta seccion extendemos los resultados de dicho experimento, teniendo en cuenta la composicion de modelos suministrada por el clasificador implementado en la seccion anterior. 
