...
% Predictor ARIMA
In Step Two of our proposed methodology (Section \ref{Sec:ModelRepresentatives}), we generate an ARIMA model for each representative obtained from the domain partitioning. %In Section \ref{Sec:TheoryARIMA} is described its theoretical aspects, and in order to find the suitable parameters $(p,d,q)$ required for the ARIMA model (such that minimizes the AIC), we use an implementation based on the \texttt{auto.arima} routine\footnote{Proposed by R. Hyndman \cite{Hyndman2008}}. 
...
\subsubsection{Analyzing Forecast Error of Predictors on Representatives}
\label{Sec:AnalyzeForecastErrors}

The previous section described the process of calculating a multi-step prediction for the future values of a time series, using a predictive model trained at a representative. Given that the out-of-sample interval of length $t_f$ is known from the dataset, we can then presents results on the corresponding forecast error. This corresponds to the Forecast Error presented in Step Three of the proposed methodology (Figure \ref{Fig:OverviewMethodology} in Chapter \ref{Chapter:Methodology}).
...
% GM review paragraph
%Comparison of forecast prediction intervals for the representative element model when is used in another element in their group. 
...
\subsubsection{Predictive Quality of Predictor on the Representative in a Model Composition}
