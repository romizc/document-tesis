...
\begin{algorithmic}[1] 
\Function{computeOffline} {$D,k\_list $}

\For{$k\ \in\ k\_list$}

 /* cluster the dataset D and obtain the set of groups and associated medoids  /*
 \State {$(medoid\_list, annotated\_ts\ \leftarrow\ cluster(D,k)$}
 \State{$medoid\_out \gets \bot$}
 
 \For{$ m\  \in\  medoid\_list$}
 
 /* Train a model at time-series m /*
 \State {$model \gets train(m) $}
 \State {$error \gets model.error $}
 
 /* Annotate the medoid m with the trained model and its error /*
 \State {$annotate (m,model, error) $}
 \State{$Add(m, medoid\_out)$}
 \EndFor
 
 \State {$ D\_out.add(k,annotated\_ts,medoid\_out) $}
